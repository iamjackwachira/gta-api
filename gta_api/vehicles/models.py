...
class Vehicle(BaseModel):
    ...
    manufacturer = models.ForeignKey(VehicleManufacturer, on_delete=models.CASCADE)
    ...
    workshop = models.ManyToManyField(VehicleWorkshop, help_text="Workshop for modifying vehicle")
    sell_ability = models.CharField(choices=choices.SELL_ABILITY_CHOICES, blank=True, null=True, max_length=50,
                                    help_text="If the vehicle can be sold")
    stolen_sell_value = models.DecimalField(max_digits=20, decimal_places=2, null=True, blank=True,
                                            validators=[MinValueValidator(1)], help_text="Max sell value if stolen")
    resale_value = models.DecimalField(max_digits=20, decimal_places=2, null=True, blank=True,
                                       validators=[MinValueValidator(1)], help_text="Max sell value if purchased")
    top_speed = models.DecimalField(max_digits=5, decimal_places=2, validators=[MinValueValidator(1)])
    based_on = models.ManyToManyField(RealLifeVehicle, help_text="Reallife vehicle it's based on")
